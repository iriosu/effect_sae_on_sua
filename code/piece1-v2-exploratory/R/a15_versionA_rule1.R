# a15: DID-ALLG-WT1-A-2026-09-01 — version A under Rule 1 weighting (each
# class-year weighted by its own entrant count), studentized exact RI;
# companion to the locked Rule 2 record run (a13). Derived from a13;
...
CP[, w_ent := as.numeric(n_ent)]   # Rule 1: own-year size
...
cls[, w_ent := as.numeric(n_ent)]  # Rule 1: own-year size
...
stopCluster(cl)
R <- rbindlist(RES)
fwrite(R, file.path(xdir, "output", "versionA_rule1_results.csv"))
say("total %.1f min", (proc.time() - t0)[["elapsed"]] / 60)

say("pre-treatment means, Rule 1 weights:")
say("  gpa %.4f | prio %.4f",
    CP[is_priv == 0L & yr < G & !is.na(gpa_z), weighted.mean(gpa_z, w_ent)],
    CP[is_priv == 0L & yr < G & !is.na(sh_prio), weighted.mean(sh_prio, w_ent)])
for (y in c("grad_ontime", "applied", "assigned", "enrolled"))
  say("  %s %.4f", y, cls[is_priv == 0L & proc < G, weighted.mean(get(y), w_ent)])
