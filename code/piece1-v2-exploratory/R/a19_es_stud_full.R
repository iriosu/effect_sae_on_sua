...
# Streams selected via command-line args: a19 <version A|B> <rule 1|2>.
...
stopifnot(VER %in% c("A", "B"), RULE %in% c("1", "2"), N_CORES >= 1L)
...
if (RULE == "2") CP[, w_ent := mean(n_ent), by = cell_id] else CP[, w_ent := as.numeric(n_ent)]
...
if (RULE == "2") cls[, w_ent := mean(n_ent), by = cell_id] else cls[, w_ent := as.numeric(n_ent)]
