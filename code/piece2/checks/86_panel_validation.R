# 86_panel_validation.R -- content validation of Piece 2/lottery_panel.rds, every
# column against its source, full population (395,950 rows), no sampling. A nonzero
# count in any check is an error in the panel. Read-only on all inputs.
...
P2<-if(file.exists("lottery_panel.rds")) "." else
    if(file.exists(file.path("..","lottery_panel.rds"))) ".." else
...
P<-as.data.table(readRDS(file.path(P2,"lottery_panel.rds")))
...
exp_cols<-c("proc","MRUN","key","RBD","won","PREF","loteria","vintage","rbd_a",
            ...
            "sch_year","prioritario","female","gpa8_z",
            "at_y1","at_y2","at_y3","at_y4","yrs_at_school","at_assigned","in_any",
            "grad_ontime")
...
bad<-bad+ok("won outside {0,1} / outcomes outside {0,1}",
  ...
  sum(!P$tracked%in%0:1)+sum(!P$grad_ontime%in%0:1))
bad<-bad+ok("PREF < 1 or loteria <= 0", P[PREF<1L | loteria<=0,.N])
bad<-bad+ok("proc outside 2016-2021", P[!proc%in%2016:2021,.N])
...
D1<-SAE$d1[cod_nivel==9L & !is.na(rbd_admitido)]
...
# ---- outcomes against the outcome table (full join) -------------------------
q<-CO[data.table(mrun=P$MRUN,proc=P$proc+5L),on=.(mrun,proc)]
...
C1<-SAE$c1[cod_nivel==9L,.(proc,MRUN=as.integer(mrun),RBD=rbd,PREF=as.integer(preferencia_postulante),
...
xg<-G8[P,on=.(MRUN,AGNO=proc),x.z]
bad<-bad+ok("gpa8_z NA-status disagrees with the graded panel", sum(is.na(xg)!=is.na(P$gpa8_z)))
bad<-bad+ok("gpa8_z value disagrees with the graded panel",
            sum(!is.na(xg)&!is.na(P$gpa8_z)&abs(xg-P$gpa8_z)>1e-10))
rm(G8,xg); invisible(gc())

# ---- take-up and persistence flags against the census (full recomputation) --
MATC<-readRDS(dp("matricula_2011_2023.rds"))
MATS<-readRDS(dp("matricula_2024_2025.rds"))
ENT<-unique(rbind(MATC[agno%in%2017:2023,.(agno,MRUN,RBD)],MATS[,.(agno,MRUN,RBD)]))
rm(MATC,MATS); invisible(gc())
for(k in 0:3){
  ee<-data.table(MRUN=P$MRUN,RBD=P$RBD,agno=P$proc+1L+k)
  xk<-as.integer(!is.na(ENT[ee,on=.(MRUN,RBD,agno),which=TRUE]))
  bad<-bad+ok(sprintf("at_y%d disagrees with the census",k+1L), sum(xk!=P[[paste0("at_y",k+1L)]]))
}
bad<-bad+ok("yrs_at_school != sum of the four year flags",
            sum(P$yrs_at_school!=P$at_y1+P$at_y2+P$at_y3+P$at_y4))
ea<-data.table(MRUN=P$MRUN,RBD=P$rbd_a,agno=P$proc+1L)
x2<-fifelse(is.na(P$rbd_a),NA_integer_,as.integer(!is.na(ENT[ea,on=.(MRUN,RBD,agno),which=TRUE])))
bad<-bad+ok("at_assigned disagrees with the census",
            sum((is.na(x2)!=is.na(P$at_assigned))|(!is.na(x2)&!is.na(P$at_assigned)&x2!=P$at_assigned)))
...
bad<-bad+ok("at_assigned NA-status != rbd_a NA-status", sum(is.na(P$at_assigned)!=is.na(P$rbd_a)))
rm(ENT,E2,ea,x2,x3); invisible(gc())
...
xgo<-as.integer(!is.na(GPm[data.table(MRUN=P$MRUN,agno=P$proc+4L),on=.(MRUN,agno),which=TRUE]))
...
say("")
if(bad==0L) say("PANEL VALIDATION: ALL CHECKS 0 -- CLEAN | %.1f min",(proc.time()-t0)[["elapsed"]]/60) else
  say("PANEL VALIDATION: %d ERROR CLASSES -- DO NOT SHIP | %.1f min",bad,(proc.time()-t0)[["elapsed"]]/60)
